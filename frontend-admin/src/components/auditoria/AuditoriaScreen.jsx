...
export default function AuditoriaScreen() {
  ...
  useEffect(() => {
    api.auditLogs().then(setRows).catch((e) => setError(e.message));
  }, []);
          ...
          <tbody>
            {filteredRows.map((r) => (
              <tr key={r.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                <td style={{ padding: '8px' }}>{new Date(r.fecha_hora).toLocaleString()}</td>
                <td style={{ padding: '8px', fontWeight: 'bold' }}>{r.accion}</td>
                <td style={{ padding: '8px' }}>{r.email_usuario || "-"}</td>
                <td style={{ padding: '8px' }}>{r.id_tienda || "-"}</td>
              </tr>
            ))}
            {filteredRows.length === 0 ? (
              <tr>
                <td colSpan={4} style={{ padding: '20px', textAlign: 'center', color: '#9ca3af' }}>No hay eventos para esa busqueda.</td>
              </tr>
            ) : null}
          </tbody>
        </table>
