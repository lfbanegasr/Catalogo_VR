...
export default function TiendasScreen() {
  ...
  const [form, setForm] = useState({ nombre_tienda: "", slug: "", whatsapp_number: "", dominio_personalizado: "", currency_symbol: "S/", activa: true });
    ...
    <div className="stack">
          ...
          <table style={{ width: '100%', minWidth: '700px', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr>
                ...
                <th style={{ padding: '14px' }}>Moneda</th>
            ...
            <tbody>
              {rows.map((r) => <tr key={r.id_tienda}>
                ...
                <td style={{ padding: '14px', fontWeight: 'bold' }}>{r.currency_symbol || "S/"}</td>
        ...
        {editing ? <div className="inline-editor"><h4>Editar tienda</h4>
          <div className="grid-form">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
              ...
              <label>Moneda<input value={editing.currency_symbol || ""} onChange={(e) => setEditing((s) => ({ ...s, currency_symbol: e.target.value }))} /></label>
            </div>
      ...
      <Card title="Crear tienda">
        <form className="grid-form" onSubmit={async (e) => {
          e.preventDefault();
          try {
            await api.adminCreateTienda({ ...form, slug: form.slug || null, whatsapp_number: form.whatsapp_number || null, dominio_personalizado: form.dominio_personalizado || null, currency_symbol: form.currency_symbol || "S/" });
            setForm({ nombre_tienda: "", slug: "", whatsapp_number: "", dominio_personalizado: "", currency_symbol: "S/", activa: true });
            await load();
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            ...
            <label>Moneda<input placeholder="Ej. S/, Bs, $" value={form.currency_symbol} onChange={(e) => setForm((s) => ({ ...s, currency_symbol: e.target.value }))} /></label>
          </div>
